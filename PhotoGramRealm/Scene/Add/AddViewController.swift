...
final class AddViewController: BaseViewController {
    ...
    @objc func saveButtonClicked() {
        ...
        let task = Diary(
            ...
            date: Date(),
            mainContents: contentTextView.text,
            photoURL: fullURL, isLike: false
        )
        
        repository.createItem(task)
        
        if let userImage = userImageView.image {
            saveImageToDocument(fileName: "andy_\(task._id).jpg", image: userImage)
        }
